#!/usr/bin/env python3
"""Run FastShop deployment commands through the sibling FastDevOps control plane."""
...
for env_path in (CONTROL / ".env", ROOT / ".env"):
    for key, value in load_local_env(env_path).items():
...
if not service:
    ...
    service = next(
        (
            ...
            if spec.get("repo", "").lower() in remote.lower()
        ),
        None,
    )
if not service:
    raise SystemExit("FastShop is not declared in FastDevOps")
if len(sys.argv) < 2:
