...
def main():
    ...
    with sync_playwright() as pw:
        ...
        if args.write_private_fixture:
            ...
            page.set_viewport_size({'width': 1440, 'height': 1000})
            page.goto(fixture + '/demo')
            page.get_by_role('button', name='Start private demo', exact=True).click()
            sample = page.locator('.e-card').filter(has=page.get_by_role('heading', name='Sample tea · Original', exact=True))
            sample.locator('[name=subscription]').select_option('on')
            sample.get_by_role('button', name='Set bag quantity', exact=True).click()
            ...
            page.screenshot(path=str(out / 'private-simulated-checkout.png'))
            ...
            page.get_by_role('button', name='Mark simulated shipped', exact=True).click()
            page.get_by_role('button', name='Mark simulated delivered', exact=True).click()
            page.get_by_role('link', name='Local inbox', exact=True).click()
            expect(page.get_by_role('heading', name='Local demo inbox', exact=True)).to_be_visible()
            page.screenshot(path=str(out / 'private-simulated-inbox.png'))
            report['checks'].append('Private simulator: newsletter confirmation/offer, checkout, account, skip/pause/resume/cancel, tracking and local inbox')
